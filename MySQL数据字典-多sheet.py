...
def generate_file(filename,db,tables,fields):
    ...
    for i in tables:        
        ...
        for j in fields[i['name']]:
            data.append(('',row,j['Field'],j['Type'],j['Null'],j['Key'],j['Extra'],j['Comment']))
            row += 1
            
        headers = [
            ('返回',),
            ('','{} （{}）'.format(i['name'],i['comment']),),
            ('','序号','字段名','类型（长度）','Null','Key','其它','备注')
        ]
        xls.write_data(i['comment'],headers + data)
        
        
        # 设置格式
        ws = xls.workbook[i['comment']]   
        ws.merge_cells('B2:H2') 
        for row in range(1,row+3):
        ...
        xls.workbook['目录'].cell(row2,4).hyperlink = "#'{}'!B2".format(i['comment'])
